...
export const useCartStore = defineStore('cart', {
    ...
    getters: {
        ...
        getHumanizedItemsCount: (state: ICartStoreState): string => {
            return state.items.length > 9 ? '9+' : state.items.length.toString()
        },
        ...
        getOrderItems: (state: ICartStoreState): { id: number, quantity: number }[] => state.items.map(({
                                                                                                            id,
                                                                                                            quantity
                                                                                                        }: ICartItem) => {
            return {id, quantity}
    ...
    actions: {
        ...
        async cancelCoupon(code: string): Promise<void> {
